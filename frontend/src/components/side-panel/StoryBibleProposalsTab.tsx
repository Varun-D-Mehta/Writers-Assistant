...
export default function StoryBibleProposalsTab({
    ...
    try {
      ...
      setProposals((prev) => [
        ...prev,
        {
          ...
          entryIndex: result.entry_index,
          currentEntry: result.current_entry,
          proposedEntry: result.proposed_entry,
          proposalType: result.proposal_type,
          sourceLabel: instruction.trim().slice(0, 40),
        },
      ]);
      setInstruction("");
      setShowForm(false);
  ...
  function handleAccept(proposal: BibleProposal) {
    onAcceptEntry(proposal.entryIndex, proposal.proposedEntry);
      ...
      <div className="border-b border-slate-700/50 p-3">
        {showForm ? (
          ...
          <button
            ...
            className="w-full rounded-lg border border-dashed border-slate-600 px-4 py-2.5 text-sm text-slate-400 transition hover:border-blue-500/50 hover:text-blue-400 disabled:opacity-50"
          >
              ...
              <div key={p.id} className="rounded-xl border border-slate-700 bg-slate-800 p-4">
                <div className="mb-2 flex items-center gap-2">
                  <span className="rounded-full bg-blue-500/20 px-2 py-0.5 text-[10px] font-medium text-blue-400">
                    {STORY_BIBLE_TYPES.find((t) => t.value === p.proposalType)?.label || p.proposalType}
                  </span>
                  <span className="text-xs text-slate-500">
                    {(p.currentEntry.name as string) || `Entry ${p.entryIndex + 1}`}
                  </span>
                </div>
                {renderEntryDiff(p.currentEntry, p.proposedEntry)}
